...
export class ChannelLog {
  async added(savedBot: BotDocument) {
    await this.sendLog('Bot Added', `<@!${savedBot.ownerId}> added <@!${savedBot.id}>.`, HexColor.Green);
  }
  async edited(savedBot: BotDocument) {
    await this.sendLog('Bot Edited', `<@!${savedBot.ownerId}> edited <@!${savedBot.id}>.`, HexColor.Green);
  }
